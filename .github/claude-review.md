...
You run **in parallel with CI**, so its jobs may still be in flight — but whether the server builds, its tests pass, and the package assembles is settled by CI and branch protection before anything merges. That is not your job. **Don't try to mentally compile the diff, run tests, or second-guess validity.** Spend your attention on what the test suite can't catch.

## What this repo is

`ghul-mcp` is an MCP server exposing semantic queries against ghūl projects — diagnostics, hover, definition, references, implementations, symbol search — by driving the compiler's analysis mode over its IPC protocol. Written in ghūl, consuming the `ghul.analysis.protocol` client library. It keeps a pool of analyser sessions (up to 8) keyed by canonical project path, reaping least-recently-used entries above the cap.

Its consumers are AI coding agents, which cannot tell a wrong answer from a right one. A query that silently returns stale or empty results is worse than one that errors, because the agent proceeds confidently on it.

## Severity bar

Raise a finding only when it would change what a careful maintainer does. In rough order of what matters here:

- **Silently wrong answers.** A query returning stale results after an edit, an empty result where a symbol exists, or results from the wrong project. The server re-reads files whose mtime moved before answering — flag anything that could bypass that.
- **Session pool correctness.** Leaks, reaping a session still in use, or two projects colliding on one key. A crashed analyser must be respawned rather than left poisoning subsequent queries.
- **Protocol conformance.** Malformed JSON-RPC, missing error responses, or a tool result shape that doesn't match its declared schema. Coordinates are 1-based, matching compiler diagnostics.
- **Blocking and timeouts.** A query that can hang without a timeout stalls the calling agent indefinitely.
...
- `UPPER_SNAKE_CASE` for concrete classes, structs, variants and enum members; `PascalCase` for namespaces, traits, abstract classes, unions and enums; `snake_case` for variables, functions, methods and properties. A class with only static members takes either. A leading underscore marks a member or type non-public.
- Imported .NET names map three ways, not one: method, property and field names become `snake_case`; enum names and members become `MACRO_CASE`; class, struct and trait names are left exactly as .NET spells them, generic arity suffix and all (`` KeyValuePair`2 ``).
